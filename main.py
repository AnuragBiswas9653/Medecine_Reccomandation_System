...
# Load model
# svc = pickle.load(open('models/RandomForest.pkl', 'rb'))
svc = None
with open('./models/RandomForest.pkl', 'rb') as model:
    randomForest = pickle.load(model)

# Helper function
def helper(dis_param):
    dis = dis_param
    try:
    ...
    pre = precautions[precautions['Disease'] == dis].iloc[0] if not precautions[precautions['Disease'] == dis].empty else []
    print("pre:",pre)
    ...
    return desc, pre[2:], med, die, wrkout
...
diseases_list = {
    ...
    31: 'Osteoarthritis', 5: 'Arthritis', 0: 'Vertigo', 2: 'Acne', 38: 'Urinary tract infection',
...
def get_predicted_value(patient_symptoms):
    ...
    for i in range(len(patient_symptoms)):
        if patient_symptoms[i] in symptoms_dict:
            input_vector[i] = symptoms_dict[patient_symptoms[i]]
    print([input_vector])
    prediction = randomForest.predict([input_vector])
    print(prediction[0])
    return prediction[0]
...
def predict():
    symptoms1 = request.form.get('symtoms1', '').strip()
    symptoms2 = request.form.get('symtoms2', '').strip()
    symptoms3 = request.form.get('symtoms34', '').strip()

    print(symptoms1, symptoms2, symptoms3)

    
    
    # if symptoms1.lower() or symptoms2.lower() or symptoms3.lower() == "symptoms":
    #     message = "Please enter valid symptoms."
    #     return render_template('index.html', message=message)
    
    # user_symptoms = [s.strip() for s in symptoms.split(',') if s.strip()]
    
    # symptoms1 = symptoms_dict.get(symptoms1, 0)
    # symptoms2 = symptoms_dict.get(symptoms2, 0)
    # symptoms3 = symptoms_dict.get(symptoms3, 0)


    user_symptoms = [symptoms1, symptoms2, symptoms3]
...
def suggestions():
    ...
    matched_symptoms = [symptom for symptom in symptoms_dict if query in symptom]
